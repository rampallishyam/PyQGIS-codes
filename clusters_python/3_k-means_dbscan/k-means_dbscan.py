...
class Ui_dlgKmeanDbscan(QDialog):
    def __init__(self):
        ...
        self.label_4.setGeometry(QtCore.QRect(20, 90, 151, 16))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        ...
        self.spinBox.setGeometry(QtCore.QRect(20, 110, 51, 22))
      
        self.spinBox.setObjectName("spinBox")
        self.btnRUN = QtWidgets.QPushButton("RUN",self)
        self.btnRUN.setGeometry(QtCore.QRect(100, 220, 71, 28))
        font = QtGui.QFont()
        font.setPointSize(-1)
        ...
        self.btnCancel.setGeometry(QtCore.QRect(190, 220, 71, 28))
        font = QtGui.QFont()
        font.setPointSize(-1)
        ...
        self.label_5 = QtWidgets.QLabel("Epsilon value: ",self)
        ...
        self.label_6 = QtWidgets.QLabel("Min. samples: ",self)
        self.label_6.setGeometry(QtCore.QRect(170, 160, 151, 16))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        ...
        self.lnEdMinSamples.setGeometry(QtCore.QRect(170, 180, 113, 20))
        self.lnEdMinSamples.setObjectName("lnEdMinSamples")
        self.lnEdEpsilon = QtWidgets.QLineEdit(self)
        self.lnEdEpsilon.setGeometry(QtCore.QRect(20, 180, 113, 20))
        ...
        self.setTabOrder(self.lnEditCsvPath, self.btnOpen)
        self.setTabOrder(self.btnOpen, self.spinBox)
        self.setTabOrder(self.spinBox, self.lnEdEpsilon)
        self.setTabOrder(self.lnEdEpsilon, self.lnEdMinSamples)
        self.setTabOrder(self.lnEdMinSamples, self.btnRUN)
        self.setTabOrder(self.btnRUN, self.btnCancel)
    ...
    def loadCsv(self):
        ...
        directory = os.path.dirname(Input_Table) + "/output/kmeans_dbscan"
    ...
    def changeLayerCrs(self, outputLayerPath):
        ...
        #
        #         qgis.core.QgsVectorFileWriter.writeAsVectorFormat(i, outputLayerDestPath,'utf-8', exp_crs, "ESRI Shapefile")
        #
        #         layerName_3857 = layerName + '_NEW_3857.shp'
        #         print (layerName_3857)
        #
        #         # Output_Layer_3857 = "c://output//" + layerName_3857  # set the filepath for the output shapefile
        #         #
        #         # print (Output_Layer_3857)
        #         outputLoad3857 = QgsVectorLayer(outputLayerDestPath, layerName_3857, 'ogr')
        #
        #         id = i.id()
        #
        #         QgsProject.instance().removeMapLayer(id)
    ...
    def getCoords(self, layer):
        ...
        layer_provider.addAttributes([QgsField(outputFieldName, QVariant.Int),QgsField(outputFieldName2, QVariant.Int)])
    ...
    def createConvHullLayer(self,path,method):
        rootPath = os.path.dirname(path)
        filename = ntpath.basename(path)
        if method=='kmeans':
            filename=filename.replace('.shp','_conv_kmeans.shp')
            layerName='convexHull_kmeans'
        else:
            filename = filename.replace('.shp', '_conv_dbscan.shp')
            layerName = 'convexHull_dbscan'
        convLayerPath = os.path.join(rootPath, filename)
        crs1=3857

        vl = QgsVectorLayer("Polygon?crs=epsg:" + str(crs1), layerName, "memory")
        pr = vl.dataProvider()
    ...
    def convHullsKmeans(self, layer, noClusters,method):

        outputFieldName = 'cluster_no'
        layerProc = QgsProject.instance().mapLayersByName(layer)[0]
        print ("CONV HULLS", layerProc.name(), noClusters, outputFieldName)

        convLayerPath = self.createConvHullLayer(layerProc.source(),method)

        vectorLyr = QgsVectorLayer(convLayerPath, 'kmeans_convexhull_'+layerProc.name(),"ogr")
    ...
    def convHullsDbscan(self, layer, noClusters,method):

        outputFieldName = 'cluster_db'
        layerProc = QgsProject.instance().mapLayersByName(layer)[0]
        print ("CONV HULLS", layerProc.name(), noClusters, outputFieldName)

        convLayerPath = self.createConvHullLayer(layerProc.source(),method)

        vectorLyr = QgsVectorLayer(convLayerPath, 'dbscan_convexhull_'+layerProc.name(),"ogr")
    ...
    def saveReport(self,layerPath, noClusters,epsilon, minSamples):
    ...
    def run(self):
        ...
        self.convHullsKmeans(mainLayer.name(),noClusters,'kmeans')
        noClusters2 = self.dbScan(mainLayer.name(), corArray, epsilon, minSamples)
        self.convHullsDbscan(mainLayer.name(), noClusters2,'db-scan')
        self.saveReport(mainLayer.source(),noClusters,epsilon,minSamples)
        print ("DONE")
        dialog.hide()
